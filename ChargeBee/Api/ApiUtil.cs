...
namespace ChargeBee.Api
{
    public static class ApiUtil
    {
        ...
        //
        // This lil farker has to go, the timeouts and connection settings need to be ported though...but will be at consumer level though...
        // private static HttpClient _httpClient = new() { Timeout = TimeSpan.FromMilliseconds(0 < ApiConfig.ConnectTimeout ? ApiConfig.ConnectTimeout : 30000) };
        ...
        private static void AddHeaders(HttpRequestMessage request, ApiConfig env)
        {
            ...
            //
            // (MM): Removed Obselete framework requirements to streamline tree-shaking.
// #if NET45
...
// #endif
        ...
        private static async Task<EntityResult> GetEntityResultAsync(String url, Params parameters, Dictionary<string, string> headers, ApiConfig env, HttpMethod meth)
        {
            ...
            {
                var result = new EntityResult(response.StatusCode, json, response.Headers);
                return result;
            }
        ...
        {
            return await GetListAsync(url, parameters, headers, env);
        }
